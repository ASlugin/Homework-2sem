...
class Program
{
    ...
    {
        positionOfEnd = 0;
        var resultChar = new char[input.Length];
        var startPositionString = new int[input.Length];
        ...
        var result = String.Concat(resultChar);
        return result;
    }
    ...
    public static void sortArrayChar(char[] inputString)
    {
        ...
        {
            for (var j = 0; j < inputString.Length - i - 1; j++)
            {
    ...
    {
        var sybmolNotFromString = (char)1;
        ...
        var indexOfNextSymbol = new int[input.Length];
